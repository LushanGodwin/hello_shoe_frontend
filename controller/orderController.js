...
$(document).ready(function () {
        ...
        return customerApi.getAllCustomers()
            .then((responseText) => {
                ...
                responseText.forEach((customer) => {
                    customerId.append(
                        `<option value="${customer.customerCode}">${customer.customerCode}</option>`
                    );
                });
            })
            .catch((error) => {
    ...
    customerId.change(function () {
        ...
        customerApi.getCustomer(custId)
            .then((responseText) => {
                ...
                $('#customerName').text(`Name: ${customer.customer_name}`);
                $('#customerContact').text(`Contact: ${customer.contact}`);
            })
            .catch((error) => {
                console.log(error);
                showError('Fetch Unsuccessful', error);
        ...
        stockApi.getAllStocks()
            .then((responseText) => {
                ...
                if (stock) {
                    ...
                    $('#availableQty').val(stock.quantity);
    ...
    $('#orderForm').submit(function (e) {
        ...
        let order = {
            ...
            id: email, // Assuming email is used as userId
        ...
        $.ajax({
            url: 'http://localhost:8080/shoe/api/v1/sale',
            method: 'POST',
